...
function getIp(done) {
    ...
    }, xhr.open("GET", settings.url_getIp + url_sep(settings.url_getIp) + "r=" + Math.random(), !0), xhr.send())
}

function dlTest(done) {
    if (tlog("dlTest"), !dlCalled) {
        dlCalled = !0;
        var totLoaded = 0, startT = (new Date).getTime(), graceTimeDone = !1, failed = !1;
        xhr = [];
        for (var testStream = function (i, delay) {
            setTimeout(function () {
                if (1 === testStatus) {
                    tlog("dl test stream started " + i + " " + delay);
                    var prevLoaded = 0, x = new XMLHttpRequest;
        ...
        interval = setInterval(function () {
            ...
            if (graceTimeDone && (dlProgress = t / (1e3 * settings.time_dl)), !(200 > t)) if (graceTimeDone) {
...
function ulTest(done) {
    if (tlog("ulTest"), !ulCalled) {
        ...
        for (var testStream = function (i, delay) {
            setTimeout(function () {
                if (3 === testStatus) {
                    ...
                    xhr[i] = x;
                    var ie11workaround;
                    if (settings.forceIE11Workaround) ie11workaround = !0; else try {
        ...
        interval = setInterval(function () {
            ...
            var t = (new Date).getTime() - startT;
            if (graceTimeDone && (ulProgress = t / (1e3 * settings.time_ul)), !(200 > t)) if (graceTimeDone) {
                var speed = totLoaded / (t / 1e3);
...
function sendTelemetry() {
    if (!(settings.telemetry_level < 1)) {
        ...
        try {
            var fd = new FormData;
            fd.append("dl", dlStatus), fd.append("ul", ulStatus), fd.append("ping", pingStatus), fd.append("jitter", jitterStatus), fd.append("log", settings.telemetry_level > 1 ? log : ""), xhr.send(fd)
        } catch (ex) {
            ...
            xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded"), xhr.send(postData)
        }
    }
}

var testStatus = -1, dlStatus = "", ulStatus = "", pingStatus = "", jitterStatus = "", clientIp = "", dlProgress = 0,
    ulProgress = 0, pingProgress = 0, log = "", settings = {
        test_order: "IP_D_U",
        time_ul: 15,
        time_dl: 15,
        time_ulGraceTime: 3,
        ...
        count_ping: 35,
        url_dl: "https://cors-anywhere.herokuapp.com/http://speedtest.uz/garbage.php",
        url_ul: "https://cors-anywhere.herokuapp.com/http://speedtest.uz/empty.php",
        url_ping: "https://cors-anywhere.herokuapp.com/http://speedtest.uz/empty.php",
        url_getIp: "https://cors-anywhere.herokuapp.com/http://speedtest.uz/getIP.php",
        ...
        useMebibits: !1,
        telemetry_level: 0,
        url_telemetry: "https://cors-anywhere.herokuapp.com/http://speedtest.uz/telemetry.php"
    }, xhr = null, interval = null, test_pointer = 0;
this.addEventListener("message", function (e) {
    ...
    if ("status" === params[0] && postMessage(testStatus + ";" + dlStatus + ";" + ulStatus + ";" + pingStatus + ";" + clientIp + ";" + jitterStatus + ";" + dlProgress + ";" + ulProgress + ";" + pingProgress), "start" === params[0] && -1 === testStatus) {
        ...
        var iRun = !1, dRun = !1, uRun = !1, pRun = !1, runNextTest = function () {
...
var ipCalled = !1, dlCalled = !1, r = new ArrayBuffer(1048576);
try {
